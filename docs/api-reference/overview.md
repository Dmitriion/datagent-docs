...
:::note[Только плейсхолдеры]
Во всех примерах `YOUR_API_KEY`, `${TOKEN}`, `${BOARD_TOKEN}`, `${COMPANY_ID}`, `${AGENT_ID}` — **выдуманные плейсхолдеры**. Не копируйте боевые ключи, webhook-секреты, IP и tenant ID в документацию и скриншоты.
:::
