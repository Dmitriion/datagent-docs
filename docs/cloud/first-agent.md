...
1. Пройдите [Быстрый старт в облаке](./getting-started) — регистрация и первичная настройка компании.
2. **Агенты → Новый агент** — имя, адаптер, модель, секреты.
3. Создайте задачу и нажмите **Запуск** — результат появится в журнале на [app.datagent.ru](https://app.datagent.ru).
