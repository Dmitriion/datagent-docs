...
# Глава 6. Битрикс24 и Телеграм как входящие
...
«Мы подключили **диалог и управление процессом** в Datagent: задачи, запуски, одобрения, единая история. Это **не полная CRM**. У моста Битрикс24 **нет** штатных инструментов вроде `bitrix24_list_leads` — агент **не выгрузит воронку** Bitrix одной кнопкой. Чтение воронки **amoCRM** — отдельный [preview-коннектор](../integrations/amocrm) (read-only tools), не этот чат-мост.»
...
Клиент остаётся в привычном **Телеграме или Битрикс24**, а вы — в Datagent с журналом и согласованиями. Ручное копирование переписки исчезает из ежедневной рутины.
