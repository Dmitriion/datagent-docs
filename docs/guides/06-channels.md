...
**Мария** живёт в **Битрикс24** и **Телеграм**: клиенты пишут туда, а копировать каждый диалог в панель вручную — лишняя работа. Её сценарий на неделю: **заявка приходит в Telegram**, агент готовит ответ, **статус уходит в Bitrix24** — без переключения между тремя окнами. «Бот в CRM без журнала» не подходит: непонятно, кто что запустил. Каналы ведут в **задачи с журналом** — с памятью, согласованиями и единой перепиской по теме.
...
**Шаг 1. Инженер настраивает мост.** На тарифе **Studio+** в Bitrix24 подключают [imbot bridge](../integrations/bitrix24): опрос `bitrix-poll`, привязка агента Datagent к линии открытых линий.  
