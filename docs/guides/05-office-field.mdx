...
**Алексей** не хочет каждый час открывать десять вкладок с задачами. Ему нужна **картина команды**: кто в работе, кто ждёт решения, где «горит». Списки Board остаются источником деталей; **Офис** даёт open-space за несколько секунд — без отдельного продукта и без замены CRM.
...
| Было | Стало |
| --- | --- |
| Обход списков агентов и задач по очереди | **Поле** + KPI-чипы за один экран |
| «Кто ждёт hire?» — неочевидно из таблиц | **Shield + N**, amber на столе при `pending_approval` |
| Разрыв «руководитель смотрит / оператор работает» | Тот же control plane, другой слой UI |

## Как пройти утро понедельника

**Шаг 1. Убедитесь, что Офис включён.** Администратор активировал **enableOffice** в экспериментальных настройках instance. Без флага пункта «Офис» в меню не будет.  
