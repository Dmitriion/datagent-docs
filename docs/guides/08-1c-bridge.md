---
title: Отчёт из 1С без ручной выгрузки
sidebar_label: '8. 1С'
description: "Финдиректор просит сводку из 1С — контролируемый мост Business+, не кнопка в задаче."
sidebar_position: 9
---

**Елена**, финансовый директор, в пятницу просит: «Сводка по дебиторке за квартал — к понедельнику». Раньше бухгалтер выгружал Excel вручную и пересылал в чат. После этой главы **Дмитрий** подключает **1С-коннектор** (**тариф Business и выше**): разработчик забирает данные через MCP, агент в задаче готовит **отчёт для Елены** — **без** прямого доступа чата к учётной базе.

:::info Тариф
Готовая **связка с 1С** — с тарифа **Business** ([тарифы](/docs/cloud/pricing)). На Studio и ниже коннектор недоступен.
:::

**Алексей** слышал: «Подключите 1С к агентам». Он объясняет честно: это **мост для разработки и контролируемых выгрузок**, не кнопка «провести документ» в карточке задачи.
...
**Шаг 4. Подключите Cursor.** Скопируйте **cursor-config** в MCP Cursor. Разработчик вызывает tools с upstream 1С **из IDE**; журнал — в привычном рабочем процессе разработки.  
...
Агент подключён к 1С через контролируемый мост — данные забирает разработчик в IDE, отчёт для финдиректора готовит команда в задаче. Больше не нужно вручную выгружать регистры каждую пятницу.

→ **Следующая глава:** [Шпаргалка по ситуациям](./playbook-index)
