...
**Елена**, финансовый директор, в пятницу просит: «Сводка по дебиторке за квартал — к понедельнику». Раньше бухгалтер выгружал Excel вручную и пересылал в чат. **Дмитрий**, инженер, подключает **1С-коннектор** (**тариф Business и выше**): разработчик в Cursor забирает данные через MCP, агент в задаче готовит **текстовый отчёт и файл** для Елены — **без** прямого доступа чата к учётной базе.
...
Финдиректор получает **отчёт в понедельник**, бухгалтер не тратит утро на ручную выгрузку, IT спокоен: 1С остаётся за MCP и Basic auth, а не за произвольным чатом.
