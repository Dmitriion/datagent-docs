...
**Мария** получает `.xlsx` с планом продаж и `.pptx` для правок. Схема «скинь в чат с моделью» ломает таблицы и слайды. После этой главы агент работает с **файлом на задаче**: вы видите **план изменений** и проходите **согласование** до применения — как в [главе про согласования](./04-trust-and-approval).
...
**Шаг 1. Подготовьте задачу.** Прикрепите `plan-may.xlsx`, назначьте «Оформителя таблиц». В описании укажите, какие листы трогать.  
...
*Рис. 3 — контекст для агента до вызова инструментов Excel.*
...
**Шаг 3. Агент планирует.** `plan_workbook_changes` с формулировкой, например: «Добавить столбец „Факт май“, формулы только на листе Summary».  
...
**Шаг 6. Контроль качества.** `validate_workbook_quality` — финальная проверка; при успехе — результат работы или комментарий на задаче.
...
## Пометки и комментарии в документе [Studio+]

Агент может оставлять **аннотации** прямо в документе на задаче — выделять важные фрагменты, добавлять вопросы к тексту, помечать места для проверки. Это не замена правок Excel через plan/apply, а **слой пометок** для согласования с коллегой до финального файла.

Доступно на тарифе **Studio и выше** ([тарифы](/docs/cloud/pricing)). На Solo и Free работа с файлами через чат и вложения остаётся, пометки в документе — нет.

## Где видна цепочка в панели

Алексей открывает задачу и видит **цепочку**: план → одобрение → файл. Не «Мария сказала, бот сделал», а журнал инструментов и запусков — удобно для аудита и разбора ошибок.
...
![Очередь согласований](/img/guides/stories/06-excel-office-03-inbox.webp)
...
- **Править исходник без одобрения** при включённом `requireApprovalForDestructive` — запуск остановится или отклонит шаг.

## Итог главы

На задаче лежит **новый файл после одобрения**, а не переписка «вот вам таблица текстом». Руководитель открывает вложение и видит тот же артефакт, что попадёт в [каталог](/docs/artifacts/overview).
