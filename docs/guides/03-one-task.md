...
**Шаг 7. Учтите одобрения.** Если агент вызывает рискованный инструмент (например применение правок в Excel), запуск может остановиться до вашего решения — см. [главу про одобрения](./04-trust-and-approval).  
...
Создайте задачу «Тест: три буллета про наш продукт» → **Запуск**. Если в ответе больше пяти строк — один комментарий с лимитом и повторный запуск. За пять минут вы проверяете цикл «задача → запуск → правка».
