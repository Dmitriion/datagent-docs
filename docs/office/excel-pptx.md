...
Раздел Office — в **тестовом режиме** ([обзор](./overview)); на **задаче** агент уже сегодня **читает** прикреплённую таблицу или презентацию, **готовит правки** (для Excel — с планом и вашим согласованием) и **прикрепляет новый файл**. Готовый Excel или PowerPoint сохраняется как [артефакт](../artifacts/overview) — **скачайте** из карточки задачи или из каталога компании.

Для работы с таблицами подключите навык из каталога (сценарии **Excel** / **xlsx** во вкладке **Сообщество**) — [навыки компании](/docs/cloud/skills). Плагин Office на задаче — с тарифа **Solo** и выше.

| Задача | Как решается |
| --- | --- |
| Агент читает структуру файла | Просмотр в плагине Office |
| Изменения под контролем | План правок → [согласование](/docs/concepts/approvals) → правки на **копии** |
| Результат в задаче | Новый прикреплённый файл → [артефакт](../artifacts/overview) |
| Безопасность | Согласование, журнал действий |

Обзор тарифов: **Чат с документом** — Solo+, **пометки в документе** — Studio+ ([Office — обзор](./overview)).

## Пример: отчёт по продажам

Попросите агента **подготовить отчёт по продажам за месяц в Excel**. Агент соберёт данные, создаст таблицу с нужными разрезами — и **прикрепит готовый файл** к задаче. Откройте **Результат** → **Скачать** или найдите файл в [каталоге артефактов](../artifacts/overview).

## Что поддерживается сейчас

| Формат | Просмотр и проверка | План правок и применение |
| --- | --- | --- |
| `.xlsx` | Да | Да |
| `.docx` | Да | Нет (только просмотр и проверка) |
| `.pptx` | Да | Нет (только просмотр и проверка) |
