---
id: bitrix24
title: Как подключить Битрикс24 к ИИ-агентам Datagent
sidebar_label: Битрикс24
description: "Битрикс24 + GigaChat в Datagent: агент отвечает в чатах CRM, готовит ответы, фиксирует в задаче. Пошагово за 10 минут."
---

# Битрикс24

:::note[Приложения Маркетплейса]
Приложения **EDPortal**, **Заявки PRO** и **Datagent Connector** живут в разделе [Приложения](/docs/apps). Эта страница — чат-мост агента Datagent с CRM, не документация тех приложений. **Datagent Connector** — транспорт Telegram, VK и MAX в Открытые линии; оператор отвечает в штатном чате линий, без задач агента в панели.
:::

:::info[Studio — ответы клиентам прямо в CRM]
**Studio** (3 900 ₽/мес) — агент читает чаты Битрикс24 и открытые линии, готовит ответ в задаче Datagent и отправляет его в CRM после вашего одобрения. [Тарифы →](../cloud/pricing)
:::

Агент подключается к вашему **Битрикс24**: принимает сообщения из **чатов CRM и открытых линий**, готовит ответ в **задаче Datagent** и отправляет одобренный текст обратно в CRM. Сотрудники пишут боту в привычном интерфейсе Битрикс24 — работа агента видна в панели с журналом шагов.

**Первый шаг:** [app.datagent.ru](https://app.datagent.ru) → **Менеджер плагинов** → **Битрикс24** → установить плагин.

![Менеджер плагинов — установка интеграций](/img/guides/stories/06-excel-office-04-plugins.webp)

:::tip[Честно о возможностях]
Плагин рассчитан на **чаты и ботов**, не на полную выгрузку воронки сделок одной кнопкой. Сценарии со сделками вне чата — через [браузер](../browser/overview) (**Studio+**), доработку под ваш процесс или отдельный [amoCRM preview](./amocrm) (чтение API, если CRM — amoCRM).
:::

## Что умеет агент в Битрикс24

- Принимать сообщения из чатов CRM и **открытых линий**
- Готовить ответы клиентам в задаче Datagent — с журналом каждого шага
- Отправлять одобренный ответ обратно в чат Битрикс24
- Работать с вложениями в переписке (если администратор CRM выдал нужные права доступа)

## Как подключить

1. Установите плагин **Битрикс24** в **Менеджере плагинов**.
2. В **Настройках компании → Битрикс24** укажите **адрес портала** и **ключ доступа** из Битрикс24 (*Приложения → Вебхуки → входящий* — обычно выдаёт администратор CRM).
3. **Зарегистрируйте бота**, выберите **агента** (рекомендуем [GigaChat](./gigachat)), нажмите **Запустить мост** → **Проверить соединение**, затем напишите боту тестовое сообщение в Битрикс24.

Пошагово с картинками — [учебник: каналы](../guides/06-channels) · [практический сценарий](../tutorials/automate-crm).

## Пример инструкции агенту

```text
Ты ассистент в чате Битрикс24. Контекст — в задаче и комментариях.
Отвечай кратко по-русски. Не выдумывай вызовы API CRM.
```

## Частые вопросы

**Нужен ли программист?**  
Первую связку часто делает администратор Битрикс24 и ответственный за Datagent. Сложные воронки — с разработчиком.

**Агент сам выгружает все лиды?**  
**Нет.** Плагин — **чаты**, не полная выгрузка CRM.

**Почему нет ответа в чате?**  
Проверьте: мост включён, запуск завершился, нет зависшего [согласования](../concepts/approvals).

**Какой тариф нужен?**  
**Studio** и выше — как в [таблице каналов](../concepts/channels).

## Что дальше

→ [Каналы](../concepts/channels)

:::note[Для инженеров]

Плагин `datagent.bitrix24`, polling `imbot.v2.Event.get`, issues + wakeup. **Нет** agent tools `bitrix24_*` в manifest. CRM (`crm.lead.*`, `crm.deal.*`) **не** вызывается.

```mermaid
flowchart LR
  User[Пользователь в Bitrix чате] --> B24[Bitrix24 imbot]
  B24 --> Poll[Plugin worker poll]
  Poll --> Srv[server]
  Srv --> Agent[Агент Datagent]
  Agent --> Srv
  Srv --> Poll
  Poll --> B24
```

Установка (on-premise / dev):

```bash
pnpm --filter @datagent/plugin-bitrix24 build
pnpm datagent plugin install packages/plugins/bitrix24
```

См. [Архитектура](../concepts/agent-architecture), [API](../api-reference/overview).

:::
