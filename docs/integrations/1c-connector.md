...
Плагин **1С Коннектор** в [app.datagent.ru](https://app.datagent.ru) поднимает прокси между опубликованной базой 1С и Datagent. Ничего не ставите на сервер Datagent — настраиваете публикацию HTTP в 1С и URL в панели компании.

Подходит компаниям на **Business**, которым нужен управляемый мост 1С ↔ команда без локального форка.

### Через IDE / MCP [для разработчиков]

Для интеграторов и 1С-разработчиков: после настройки плагина копируете фрагмент **`mcp.json`** в **Cursor** (или другую IDE с MCP) и вызываете tools 1С из редактора. Агент в **задаче Datagent** эту 1С **напрямую не дергает** без отдельной доработки.
...
Плагин работает в облаке на [app.datagent.ru](https://app.datagent.ru); on-premise — по [Enterprise](../cloud/on-premise). Тариф — **Business** или выше.
