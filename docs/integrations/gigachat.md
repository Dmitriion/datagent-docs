...
> **Для подключения GigaChat нужен API-ключ от Сбера (Client ID и Client Secret).**  
> **Ограничений по тарифу Datagent нет** — подключается на **любом** плане, включая **Free**. Оплата вызовов модели — по тарифам Сбера; Datagent списывает **запуски** платформы.
...
5. Каждый запуск списывает **запуски** с лимита тарифа; журнал шагов — в задаче.
...
| **С инструментами** | Таблицы, [браузер](../browser/overview) (**Studio+**) — если включены плагины |
...
Ключи и квоты — у **Сбера** по их тарифу. **Datagent** списывает **запуски** с лимита вашего плана.
