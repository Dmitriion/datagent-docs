...
Клиенты пишут в **Битрикс24** и **Телеграм** — привычные мессенджеры. Вы работаете в Datagent: каждое сообщение становится **задачей** с перепиской, запусками агента и при необходимости **подтверждением** рискованных шагов.

Канал связи **не заменяет CRM**: сделки остаются в Битрикс24. Datagent ведёт **диалог**, **агента** и **контроль**.
...
Клиент пишет в открытую линию **Битрикс24**: «Когда будет счёт?» В Datagent появляется **задача** с текстом сообщения. Оператор запускает агента — тот смотрит сделку в CRM и готовит ответ. После [согласования](./approvals) ответ уходит в ту же линию; в журнале запуска видно, какие шаги выполнил агент.

## Каналы связи и тарифы
...
- Рискованные шаги — [подтверждения](./approvals); в Телеграм могут прийти **уведомления**.
...
| Секреты | Webhook и токены в настройках плагина | Токен бота — [ключи доступа](./secrets) |
