...
Представьте обычный рабочий день: вы формулируете поручение в **задаче**, назначаете **агента** и нажимаете **Запустить**. Агент читает контекст компании, обращается к **GigaChat** или **YandexGPT**, при необходимости открывает таблицу или отвечает в CRM — и возвращает результат в **журнал задачи**. Если шаг рискованный (правка файла, кнопка на сайте), агент **спросит вас** в разделе подтверждений.

Всё в браузере на [app.datagent.ru](https://app.datagent.ru) — без своего сервера.
...
![Очередь согласований — контроль рискованных действий](/img/guides/stories/03-approval-flow-02-inbox.webp)
...
| **Канал связи** | Сообщение в Битрикс24 или Телеграм → задача → запуск |
...
| **Подтверждения** | Очередь «можно ли сделать?» |
