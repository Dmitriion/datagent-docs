...
Когда вы **запускаете агента** в Datagent, сервер выполняет цикл: обращается к нейросети (GigaChat, YandexGPT и др.), при необходимости вызывает **инструменты** плагинов (браузер, интеграции) и повторяет шаги, пока не будет готов ответ или не сработает лимит.

Для вас это выглядит просто: **задача → Запуск → журнал**. Ниже — схема для тех, кто хочет понять цепочку глубже. Карта компонентов — в [Архитектуре](./agent-architecture.md).
