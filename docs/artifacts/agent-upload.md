...
Когда agent сохраняет отчёт, скриншот или таблицу, файл уходит через **API вложений** во время run. Вы видите его в **Output** на задаче и в [каталоге артефактов](./overview).
...
В финальном комментарии к задаче укажите **ссылку на артефакт**, а не только путь на машине agent.
...
| 403 | Ключ agent другой компании |
...
- [Прочитать обзор REST API](/docs/api-reference/overview) — ключи agent
- [Разобраться с задачами и Output](/docs/concepts/issues) — куда попадает результат
