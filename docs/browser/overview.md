...
> **Зачем:** Открыть сайт, нажать кнопку или прочитать страницу на **Вашем** компьютере — не в абстрактном облачном браузере. **Управление браузером** связывает [app.datagent.ru](https://app.datagent.ru) с локальным Chromium через plugin и службу `datagent-bridge`.
